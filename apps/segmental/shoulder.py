...
def g(arr: np.ndarray, code: str) -> float:
    """
    arr, 'AL1' → arr[row=0, col=col_letters_to_index('AL')]
...
def _shoulder_center(arr: np.ndarray, i: int) -> np.ndarray:
    """
    프레임 i 에서 어깨 중심 좌표 (cm) 반환
    왼쪽 어깨:  AL_i, AM_i, AN_i
    오른쪽 어깨: BA_i, BB_i, BC_i
    """
    L = np.array([g(arr, f"AL{i}"), g(arr, f"AM{i}"), g(arr, f"AN{i}")])
    R = np.array([g(arr, f"BA{i}"), g(arr, f"BB{i}"), g(arr, f"BC{i}")])
    return (L + R) / 2

def _shoulder_vector(arr: np.ndarray, i: int) -> np.ndarray:
    """
    프레임 i 에서 양측 어깨 벡터 R - L 반환
    """
    L = np.array([g(arr, f"AL{i}"), g(arr, f"AM{i}"), g(arr, f"AN{i}")])
    R = np.array([g(arr, f"BA{i}"), g(arr, f"BB{i}"), g(arr, f"BC{i}")])
    return R - L
...
def compute_shoulder_tdd_table(
    pro_path: Path,
    golfer_path: Path,
    deg2m: float = 0.01
) -> pd.DataFrame:
    ...
    for i in range(1, N):
        # 이동 거리 (cm→m)
        d1 = np.linalg.norm(_shoulder_center(arr1, i+1) - _shoulder_center(arr1, i)) / 100
        d2 = np.linalg.norm(_shoulder_center(arr2, i+1) - _shoulder_center(arr2, i)) / 100
        ...
        v1, v1p = _shoulder_vector(arr1, i),   _shoulder_vector(arr1, i+1)
        v2, v2p = _shoulder_vector(arr2, i),   _shoulder_vector(arr2, i+1)
...
def compute_shoulder_rotation_table(
    pro_path: Path,
    golfer_path: Path
) -> pd.DataFrame:
    """
    어깨 수평/수직 회전각 분석 테이블 반환
    • index: ['1-2','2-3',…,'9-10','1-4','4-7','7-10','Total']
    • columns:
    ...
    for i in range(1, N):
        # --- 수평 회전각 (XZ 평면) ---
        vp1 = _shoulder_vector(arr_p, i)[[0,2]]
        vp2 = _shoulder_vector(arr_p, i+1)[[0,2]]
        vh1 = _shoulder_vector(arr_h, i)[[0,2]]
        vh2 = _shoulder_vector(arr_h, i+1)[[0,2]]
...
if __name__ == "__main__":
    ...
    print("▶ 어깨 TDD 분석 ▶")
    print(compute_shoulder_tdd_table(pro, golf).to_string(), "\n")
    print("▶ 어깨 회전각 분석 ▶")
    print(compute_shoulder_rotation_table(pro, golf).to_string())
