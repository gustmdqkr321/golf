...
def _club_center(arr: np.ndarray, i: int) -> np.ndarray:
    """
    프레임 i 에서 클럽 중심 좌표 (cm) 반환
    • 한쪽 끝: AX_i, AY_i, AZ_i
    • 다른 쪽 끝: CN_i, CO_i, CP_i
    """
    A = np.array([g(arr, f"AX{i}"), g(arr, f"AY{i}"), g(arr, f"AZ{i}")])
    B = np.array([g(arr, f"CN{i}"), g(arr, f"CO{i}"), g(arr, f"CP{i}")])
    return (A + B) / 2

def _club_vector(arr: np.ndarray, i: int) -> np.ndarray:
    """
    프레임 i 에서 양 끝점 벡터 B - A 반환
    """
    A = np.array([g(arr, f"AX{i}"), g(arr, f"AY{i}"), g(arr, f"AZ{i}")])
    B = np.array([g(arr, f"CN{i}"), g(arr, f"CO{i}"), g(arr, f"CP{i}")])
    return B - A
...
def compute_club_tdd_table(
    ...
    for i in range(1, N):
        # 이동 거리 (cm→m)
        d1 = np.linalg.norm(_club_center(arr1, i+1) - _club_center(arr1, i)) / 100
        d2 = np.linalg.norm(_club_center(arr2, i+1) - _club_center(arr2, i)) / 100
        ...
        v1, v1p = _club_vector(arr1, i),   _club_vector(arr1, i+1)
        v2, v2p = _club_vector(arr2, i),   _club_vector(arr2, i+1)
...
def compute_club_rotation_table(
    pro_path: Path,
    golfer_path: Path
) -> pd.DataFrame:
    """
    클럽 수평/수직 회전각 분석 테이블 반환
    • index: ['1-2','2-3',…,'9-10','1-4','4-7','7-10','Total']
    • columns:
    ...
    for i in range(1, N):
        # --- 수평 회전각 (XZ 평면) ---
        vp1 = _club_vector(arr_p, i)[[0,2]]
        vp2 = _club_vector(arr_p, i+1)[[0,2]]
        vh1 = _club_vector(arr_h, i)[[0,2]]
        vh2 = _club_vector(arr_h, i+1)[[0,2]]
...
if __name__ == "__main__":
    ...
    print("▶ 클럽 TDD 분석 ▶")
    print(compute_club_tdd_table(pro, golf).to_string(), "\n")
    print("▶ 클럽 회전각 분석 ▶")
    print(compute_club_rotation_table(pro, golf).to_string())
