...
def _arm_center(arr: np.ndarray, i: int) -> np.ndarray:
    """
    프레임 i 에서 팔 중심 좌표 (cm) 반환
    왼팔:  AL_i, AM_i, AN_i
    오른팔: AX_i, AY_i, AZ_i
    """
    L = np.array([g(arr, f"AL{i}"), g(arr, f"AM{i}"), g(arr, f"AN{i}")])
    R = np.array([g(arr, f"AX{i}"), g(arr, f"AY{i}"), g(arr, f"AZ{i}")])
    return (L + R) / 2

def _arm_vector(arr: np.ndarray, i: int) -> np.ndarray:
    """
    프레임 i 에서 양측 팔 벡터 R - L 반환
    """
    L = np.array([g(arr, f"AL{i}"), g(arr, f"AM{i}"), g(arr, f"AN{i}")])
    R = np.array([g(arr, f"AX{i}"), g(arr, f"AY{i}"), g(arr, f"AZ{i}")])
    return R - L
...
def compute_arm_tdd_table(
    ...
    for i in range(1, N):
        # 이동 거리 (cm→m)
        d1 = np.linalg.norm(_arm_center(arr1, i+1) - _arm_center(arr1, i)) / 100
        d2 = np.linalg.norm(_arm_center(arr2, i+1) - _arm_center(arr2, i)) / 100
        ...
        v1, v1p = _arm_vector(arr1, i),   _arm_vector(arr1, i+1)
        v2, v2p = _arm_vector(arr2, i),   _arm_vector(arr2, i+1)
...
def compute_arm_rotation_table(
    pro_path: Path,
    golfer_path: Path
) -> pd.DataFrame:
    """
    왼팔 수평/수직 회전각 분석 테이블 반환
    • index: ['1-2','2-3',…,'9-10','1-4','4-7','7-10','Total']
    • columns:
    ...
    for i in range(1, N):
        # --- 수평 회전각 (XZ 평면) ---
        vp1 = _arm_vector(arr_p, i)[[0,2]]
        vp2 = _arm_vector(arr_p, i+1)[[0,2]]
        vh1 = _arm_vector(arr_h, i)[[0,2]]
        vh2 = _arm_vector(arr_h, i+1)[[0,2]]
...
if __name__ == "__main__":
    ...
    print("▶ 왼팔 TDD 분석 ▶")
    print(compute_arm_tdd_table(pro, golf).to_string(), "\n")
    print("▶ 왼팔 회전각 분석 ▶")
    print(compute_arm_rotation_table(pro, golf).to_string())
